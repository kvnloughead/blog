---
title: Basic Grepping
tags: ['Bash', 'grep', 'Unix', 'Command Line']
---

Searching through text is a common task that programmers encounter, and most, if not all, languages provide methods for handling it. The Unix shell provides a handy tool for this purpose called `grep`. It is a powerful program, and can be daunting to get started with. Here are a few simple ways that you can start using `grep` in your daily tasks.
...
and then search through it for the flag. Maybe you remember that in a `man` page you can search for patterns using the `/` key. And actually, that works quite nicely, with the keymapping for the _n_ext and_p_revious actions being refreshingly commonsensical. But this is an article about `grep`, so let's stay on topic. Here's how we can pipe the contents of the `man` page for `curl` into `grep`
